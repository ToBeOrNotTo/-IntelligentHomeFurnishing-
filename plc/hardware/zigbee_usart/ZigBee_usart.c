...
void USART2_IRQHandler(void)
{
   ...
   if( (USART2->SR & (1<<4)) == (1<<4) ) 
   {
	 ...
	 size = ZIGBEE_BUFFER_LEN - DMA1_Channel6->CNDTR;//判断总共接收了多少个字节
	 R_Finish = 1;
...
void zigbee_usart_init(u32 pclk2,u32 bound)
{
  ...
  RCC->APB1ENR |= (1<<17);
    ...
    USART2->SR |= 0x1ff;
	MY_NVIC_Init(2,0,USART2_IRQChannel,2);
	MY_NVIC_Init(1,3,DMA1_Channel7_IRQChannel,2);
#endif
}
