...
u8 USART1_Tx_Buffer[USART_BUFFER_LEN];
volatile u16 USART1_TX_Len=0;
void USART1_IRQHandler(void)
{
   ...
   if( (USART1->SR & (1<<4)) == (1<<4) ) 
   {
	 ...
	 USART1_TX_Len =USART_BUFFER_LEN - DMA1_Channel5->CNDTR;
	 //usart_buffer.size = USART_BUFFER_LEN - DMA1_Channel4->CNDTR;//判断总共接收了多少个字节
	 Usart1_RFinish = 1;
...
void usart_init(u32 pclk2,u32 bound)
{
  ...
  USART1->CR3 |= (1<<6)|(1<<7);//DMAR DMAT
	...
	DMA1_Channel5->CMAR = (u32)(USART1_Tx_Buffer);
